...
public class InventoryData implements InventoryDataInterface {
	...
	public ItemStack[] getInventoryContents(OfflinePlayer player) {
		try {
			...
			NBTTagList nbttaglist = data.getList("Inventory");
			ItemStack[] items = new ItemStack[36];
	        for (int i = 0; i < nbttaglist.size(); ++i) {
	            NBTTagCompound nbttagcompound = (NBTTagCompound) nbttaglist.get(i);
	            int j = nbttagcompound.getByte("Slot") & 255;
	            ItemStack itemstack = CraftItemStack.asCraftMirror(net.minecraft.server.v1_6_R3.ItemStack.createStack(nbttagcompound));
	            if (itemstack != null) {
	                if (j >= 0 && j < 36) {
	                    items[j] = itemstack;
	                }
